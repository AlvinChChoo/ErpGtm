...
<script runat="server">
    ...
    Sub cmdBack_Click_1(sender As Object, e As EventArgs)
        Response.redirect("SFASApp2.aspx")
    End Sub
    ...
    Sub cmdSubmit_Click(sender As Object, e As EventArgs)
        if page.isvalid = true then
            ...
            if rbApprove.checked =true then
                ReqCOM.executeNonquery("Update SFAS_M set App2_By = '" & trim(request.cookies("U_ID").value) & "',App2_Date = '" & cdate(now) & "',App2_status = 'Y',SFAS_Status = 'APPROVED' where sfas_no = '" & trim(lblSFASNo.text) & "';")
                ReqCOM.ExecuteNonQuery("Insert into Sales_forecast(MODEL_NO,FORECAST_DATE,FORECAST_QTY,UP,AMT,FORECAST_TYPE) select MODEL_NO,FORECAST_DATE,FORECAST_QTY,UP,AMT,FORECAST_TYPE from SFAS_D where sfas_no = '" & trim(lblSFASNo.text) & "';")
    
                Dim cnnGetFieldVal As SqlConnection = New SqlConnection(ConfigurationSettings.AppSettings("ConnectionString"))
                cnnGetFieldVal.Open()
                Dim myCommand As SqlCommand = New SqlCommand("Select Seq_No from Sales_Forecast where Ref_No is null order by seq_no asc", cnnGetFieldVal )
                Dim drGetFieldVal As SqlDataReader = myCommand.ExecuteReader(CommandBehavior.CloseConnection)
    
                do while drGetFieldVal.read
                    RefNo = ReqCOm.GetDocumentNo("SF_NO")
                    ReqCOM.ExecuteNonQuery("Update Sales_Forecast set ref_no = '" & trim(RefNo) & "' where seq_No = " & drGetFieldVal("Seq_No") & ";")
                    ReqCOM.ExecuteNonQuery("Update Main set SF_No = SF_No + 1")
                loop
                ...
                ReqCOM.ExecuteNonQuery("update sales_forecast set sales_forecast.curr_code = cust.curr_code from sales_forecast,cust,model_master where sales_forecast.model_no = model_master.model_code and model_master.cust_code = cust.cust_code and sales_forecast.curr_code is null")
            elseif rbreject.checked = true then
                ReqCOM.executeNonquery("Update SFAS_M set App2_By = '" & trim(request.cookies("U_ID").value) & "',App2_Date = '" & cdate(now) & "',App2_status = 'N',SFAS_Status = 'REJECTED' where sfas_no = '" & trim(lblSFASNo.text) & "';")
            end if
    ...
    Sub cmdRefresh_Click(sender As Object, e As EventArgs)
        Response.redirect("SFASApp2Det.aspx?ID=" & Request.params("ID"))
    End Sub
