...
<script runat="server">
    ...
    Sub Page_Load(ByVal sender As Object, ByVal e As EventArgs)
        if page.isPostBack = false then
            ...
            Dim RsSource as SQLDataReader = ReqCOM.ExeDataReader("Select * from Buyer_SR_D where Seq_No = " & request.params("ID") & ";")
            ...
            Do while RSSource.read
                ...
                lblRem.text = rsSource("Rem").tostring
    ...
    Sub cmdUpdate_Click(sender As Object, e As EventArgs)
        if page.isvalid = true  then
            if trim(cmdNext.text) = "Back" then
                ...
                Dim i as integer
                Dim PRNo as integer = ReqCOM.GetFieldVal("Select sr_No from Buyer_SR_D where seq_no = " & request.params("ID") & ";","SR_No")
                ...
                For i = 0 to dtgPartSource.Items.Count - 1
                    ...
                    if cint(Quantity.text) <> 0 then
                        ...
                        StrSql = "Insert into Buyer_SR_D(sr_No,lot_no,PART_NO,eta_date,QTY_TO_BUY,PROCESS_DAYS,PR_DATE,UP,VEN_CODE,LEAD_TIME,VARIANCE,req_qty,Rem) "
                        StrSql = StrSql + "Select " & PRNo & ",'" & trim(lblLotNo.text) & "','" & trim(lblPartNo.text) & "','" & trim(lblETA.text) & "'," & cint(OrderQty.text) & "," & PRProcessingDay & ",'" & PRDate & "'," & UP.text & ",'" & trim(Supplier.text) & "'," & cint(LeadTime.text) * 7 & "," & cint(OrderQty.text) & " - " & cint(Quantity.text) & "," & cint(Quantity.text) & ",'" & trim(lblRem.text) & "';"
                        ReqCOM.ExecuteNonQuery(StrSql)
                    end if
                next
    
                ReqCOm.ExecuteNonQuery("Update Buyer_SR_D set Buyer_SR_D.Lead_Time = PS.Lead_Time * 7 from Part_Source PS,Buyer_SR_D where Buyer_SR_D.Ven_Code = PS.Ven_Code and Buyer_SR_D.Part_No = PS.Part_No and Buyer_SR_D.SR_No = '" & trim(lblSRNo.text) & "';")
                ReqCOm.ExecuteNonQuery("Update Buyer_SR_D set NET_ETA = ETA_Date - Lead_Time where  SR_No = '" & trim(lblSRno.text) & "';")
    
                ReqCOM.ExecuteNonQuery("Update Buyer_SR_D set Calculated_qty = qty_to_buy where sr_No = '" & trim(lblSRno.text) & "';")
                reqCOM.ExecuteNonQuery("Delete from Buyer_SR_D where seq_no = " & cint(request.params("ID")) & ";")
    
                SeqNo = ReqCOM.GetFieldVal("select Seq_No from Buyer_SR_M where SR_No = '" & trim(lblSRNo.text) & "';","Seq_No")
                Response.redirect("BuyerSpecialRequestDet.aspx?ID=" & SeqNo)
            End if
        End if
    End Sub
    ...
    Sub cmdCancel_Click(sender As Object, e As EventArgs)
        ...
        SeqNo = ReqCOM.GetFieldVal("select Seq_No from Buyer_SR_M where sr_No = '" & trim(lblSRNo.text) & "';","Seq_No")
        Response.redirect("BuyerSpecialRequestDet.aspx?ID=" & Request.params("ReturnID"))
    End Sub
                                    ...
                                    <tbody>
                                        <tr>
                                            <td>
                                                ...
                                                <p>
                                                    <asp:Label id="lblSRNo" runat="server" width="206px" visible="False">Label</asp:Label><asp:Label id="lblLotNo" runat="server" width="206px" visible="False"></asp:Label>
                                                </p>
                                                <p>
                                                ...
                                                <p>
                                                    <asp:Label id="lblExcess" runat="server" forecolor="Red" font-size="X-Large" font-bold="True"></asp:Label><asp:Label id="lblRem" runat="server" width="161px" cssclass="OutputText" visible="False"></asp:Label>
                                                </p>
                                                <p align="center">
