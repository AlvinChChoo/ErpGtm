...
<script runat="server">
    ...
    Sub SetCurrecntInd()
        ...
        ReqCOM.ExecuteNonQuery("Update Import_BoM_Quote set ind = 'Y' where curr_code = ''")
    
        'ReqCOM.ExecuteNonQuery("Update Import_BoM_Quote set ind = 'Y' where curr_code <> '' and curr_code in (select distinct(curr_code) from bom_Quote_Curr where Bom_Quote_no = '" & trim(request.params("BOMQuoteNo")) & "')")
    ENd Sub
    ...
    Sub cmdImport_Click(sender As Object, e As EventArgs)
        ...
        if ReqCOM.FuncCheckDuplicate("Select top 1 Part_No from Import_Bom_Quote where Ind = 'N'","Part_No") = true then
            ...
            ReqCOM.ExecuteNonQuery("insert into bom_quote_d(BOM_QUOTE_NO,MAIN_PART,PART_NO,CUST_PART_NO,PART_DESC,PART_SPEC,P_USAGE,STD_VEN_CODE,STD_VEN_NAME,MFG_MPN,STD_CURR_CODE,STD_ORI_UP,STD_LT,STD_SPQ,STD_MOQ,main_part_temp,part_no_temp) select BOM_QUOTE_NO,MAIN_PART,PART_NO,CUST_PART_NO,PART_DESC,PART_SPEC,P_USAGE,VEN_CODE,VEN_NAME,MFG_MPN,CURR_CODE,UP,LEAD_TIME,SPQ,MOQ,main_part,part_no from import_bom_quote order by seq_no asc")
            ...
            Reqcom.UpdateBOMQuoteMainPart(trim(request.params("BOMQuoteNo")))
