...
<script runat="server">
    ...
    Sub Page_Load(ByVal sender As Object, ByVal e As EventArgs)
        ...
        Sub ProcLoadGridData()
            ...
            'if trim(cmbFECNStatus.selecteditem.value) = "ALL" then StrSql = "SELECT * FROM FECN_M where App2_Date is not null and App1_date is not null AND " & trim(cmbSearch.selecteditem.value) & " like '%" & trim(txtSearch.text) & "%' order by seq_no desc"
            'if trim(cmbFECNStatus.selecteditem.value) <> "ALL" then StrSql = "SELECT * FROM FECN_M where App2_Date is not null and App1_date is not null AND " & trim(cmbSearch.selecteditem.value) & " like '%" & trim(txtSearch.text) & "%' AND FECN_STATUS = '" & TRIM(cmbFECNStatus.selecteditem.value) & "'  order by seq_no desc"

            if trim(cmbSearch.selecteditem.value) = "FECN_No" then
                if trim(cmbFECNStatus.selecteditem.value) = "ALL" then StrSql = "SELECT * FROM FECN_M where App2_Date is not null and App1_date is not null AND " & trim(cmbSearch.selecteditem.value) & " like '%" & trim(txtSearch.text) & "%' ORDER BY Sort_Seq,FEcn_No DESC"
                if trim(cmbFECNStatus.selecteditem.value) <> "ALL" then StrSql = "SELECT * FROM FECN_M where App2_Date is not null and App1_date is not null AND " & trim(cmbSearch.selecteditem.value) & " like '%" & trim(txtSearch.text) & "%' AND FECN_STATUS = '" & TRIM(cmbFECNStatus.selecteditem.value) & "' ORDER BY Sort_Seq,FEcn_No DESC"
            end if
            ...
            if trim(cmbSearch.selecteditem.value) = "PART_NO" then
                if trim(cmbFECNStatus.selecteditem.value) =  "ALL" then StrSql = "SELECT * FROM FECN_M where App2_Date is not null and App1_date is not null AND fecn_no in (Select Fecn_No from fecn_d where main_part_b4 like '%" & trim(txtSearch.text) & "%' or main_part like '%" & trim(txtSearch.text) & "%' or alt_part_b4 like '%" & trim(txtSearch.text) & "%' or alt_part like '%" & trim(txtSearch.text) & "%') ORDER BY Sort_Seq,FEcn_No DESC"
                if trim(cmbFECNStatus.selecteditem.value) <> "ALL" then StrSql = "SELECT * FROM FECN_M where App2_Date is not null and App1_date is not null AND fecn_no in (Select Fecn_No from fecn_d where main_part_b4 like '%" & trim(txtSearch.text) & "%' or main_part like '%" & trim(txtSearch.text) & "%' or alt_part_b4 like '%" & trim(txtSearch.text) & "%' or alt_part like '%" & trim(txtSearch.text) & "%') AND FECN_STATUS = '" & TRIM(cmbFECNStatus.selecteditem.value) & "' ORDER BY Sort_Seq,FEcn_No DESC"
            end if

            if trim(cmbSearch.selecteditem.value) = "LOT_NO" then
                if trim(cmbFECNStatus.selecteditem.value) =  "ALL" then StrSql = "SELECT * FROM FECN_M where App2_Date is not null and App1_date is not null AND fecn_no in (Select Fecn_No from fecn_d where lot_no like '%" & trim(txtSearch.text) & "%') ORDER BY Sort_Seq,FEcn_No DESC"
                if trim(cmbFECNStatus.selecteditem.value) <> "ALL" then StrSql = "SELECT * FROM FECN_M where App2_Date is not null and App1_date is not null AND fecn_no in (Select Fecn_No from fecn_d where lot_no like '%" & trim(txtSearch.text) & "%') AND FECN_STATUS = '" & TRIM(cmbFECNStatus.selecteditem.value) & "' ORDER BY Sort_Seq,FEcn_No DESC"
            end if
       ...
       Protected Sub FormatRow(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.DataGridItemEventArgs)
            ...
            If e.Item.ItemType = ListItemType.Item Or e.Item.ItemType = ListItemType.AlternatingItem Then
                ...
                if trim(App3Date.text) = "" and trim(ucase(Status.text)) <> "REJECTED" then e.Item.CssClass = "PartSource"
                if trim(App3Date.text) = "" and trim(ucase(Status.text)) <> "REJECTED" and trim(ucase(Urgent.text)) = "Y" then e.Item.CssClass = "Urgent"
            End if
        End Sub
                                        ...
                                        <tr>
                                            <td>
                                                <div align="center"><asp:Label id="Label1" runat="server" cssclass="OutputText">SEARCH</asp:Label>&nbsp;<asp:TextBox id="txtSearch" runat="server" CssClass="OutputText" Width="125px"></asp:TextBox>
                                                    &nbsp;<asp:Label id="Label3" runat="server" cssclass="OutputText">BY</asp:Label>&nbsp;<asp:DropDownList id="cmbSearch" runat="server" CssClass="OutputText" Width="117px">
                                                    ...
                                                    &nbsp;<asp:Label id="Label4" runat="server" cssclass="OutputText">SHOW</asp:Label>&nbsp;<asp:DropDownList id="cmbFECNStatus" runat="server" CssClass="OutputText" Width="174px">
                                            ...
                                            <td>
                                                <p>
                                                    &nbsp;<asp:DataGrid id="GridControl1" runat="server" width="100%" OnItemDataBound="FormatRow" OnSelectedIndexChanged="GridControl1_SelectedIndexChanged" PagerStyle-HorizontalAligh="Right" PagerStyle-PrevPageText="Prev" PagerStyle-NextPageText="Next" PageSize="20" OnPageIndexChanged="OurPager" AllowPaging="True" BorderColor="Black" GridLines="Vertical" cellpadding="4" ShowFooter="True" AutoGenerateColumns="False" OnSortCommand="SortGrid" AllowSorting="True">
                                                        ...
                                                        <Columns>
                                                            <asp:HyperLinkColumn Text="View" DataNavigateUrlField="seq_no" DataNavigateUrlFormatString="FECNApp3Det.aspx?ID={0}"></asp:HyperLinkColumn>
