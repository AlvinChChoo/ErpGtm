...
<script runat="server">
    ...
    Sub ProcLoadGridData()
        ...
        if trim(cmbStatus.selecteditem.value) = "ALL" then
            if trim(cmbSearchField.selecteditem.value) = "BOM_QUOTE_NO" then StrSql = "SELECT * FROM BOM_QUOTE_M WHERE " & TRIM(cmbSearchField.selecteditem.value) & " like '%" & trim(txtSearch.text) & "%' and App2_By is not null order by BOM_QUOTE_NO desc"
            if trim(cmbSearchField.selecteditem.value) = "MODEL_NO" then StrSql = "SELECT * FROM BOM_QUOTE_M WHERE " & TRIM(cmbSearchField.selecteditem.value) & " like '%" & trim(txtSearch.text) & "%' and App2_By is not null  order by BOM_QUOTE_NO desc"
            if trim(cmbSearchField.selecteditem.value) = "MODEL_DESC" then StrSql = "SELECT * FROM BOM_QUOTE_M WHERE " & TRIM(cmbSearchField.selecteditem.value) & " like '%" & trim(txtSearch.text) & "%' and App2_By is not null  order by BOM_QUOTE_NO desc"
            if trim(cmbSearchField.selecteditem.value) = "PART_SPEC" then StrSql = "SELECT * FROM BOM_QUOTE_M WHERE bom_quote_no in (Select BOM_Quote_No from bom_quote_D where " & TRIM(cmbSearchField.selecteditem.value) & " like '%" & trim(txtSearch.text) & "%' ) and App2_By is not null  order by BOM_QUOTE_NO desc"
            if trim(cmbSearchField.selecteditem.value) = "PART_DESC" then StrSql = "SELECT * FROM BOM_QUOTE_M WHERE bom_quote_no in (Select BOM_Quote_No from bom_quote_D where " & TRIM(cmbSearchField.selecteditem.value) & " like '%" & trim(txtSearch.text) & "%' ) and App2_By is not null  order by BOM_QUOTE_NO desc"
            if trim(cmbSearchField.selecteditem.value) = "CUST_part_no" then StrSql = "SELECT * FROM BOM_QUOTE_M WHERE bom_quote_no in (Select BOM_Quote_No from bom_quote_D where " & TRIM(cmbSearchField.selecteditem.value) & " like '%" & trim(txtSearch.text) & "%' ) and App2_By is not null  order by BOM_QUOTE_NO desc"
            if trim(cmbSearchField.selecteditem.value) = "PART_NO" then StrSql = "SELECT * FROM BOM_QUOTE_M WHERE bom_quote_no in (Select BOM_Quote_No from bom_quote_D where " & TRIM(cmbSearchField.selecteditem.value) & " like '%" & trim(txtSearch.text) & "%' ) and App2_By is not null  order by BOM_QUOTE_NO desc"
        elseif trim(cmbStatus.selecteditem.value) <> "ALL" then
            if trim(cmbSearchField.selecteditem.value) = "BOM_QUOTE_NO" then StrSql = "SELECT * FROM BOM_QUOTE_M WHERE " & TRIM(cmbSearchField.selecteditem.value) & " like '%" & trim(txtSearch.text) & "%' and BOM_Quote_Status = '" & trim(cmbStatus.selecteditem.value) & "' and App2_By is not null  order by BOM_QUOTE_NO desc"
            if trim(cmbSearchField.selecteditem.value) = "MODEL_NO" then StrSql = "SELECT * FROM BOM_QUOTE_M WHERE " & TRIM(cmbSearchField.selecteditem.value) & " like '%" & trim(txtSearch.text) & "%' and BOM_Quote_Status = '" & trim(cmbStatus.selecteditem.value) & "' and App2_By is not null  order by BOM_QUOTE_NO desc"
            if trim(cmbSearchField.selecteditem.value) = "MODEL_DESC" then StrSql = "SELECT * FROM BOM_QUOTE_M WHERE " & TRIM(cmbSearchField.selecteditem.value) & " like '%" & trim(txtSearch.text) & "%' and BOM_Quote_Status = '" & trim(cmbStatus.selecteditem.value) & "' and App2_By is not null  order by BOM_QUOTE_NO desc"
            if trim(cmbSearchField.selecteditem.value) = "PART_SPEC" then StrSql = "SELECT * FROM BOM_QUOTE_M WHERE bom_quote_no in (Select BOM_Quote_No from bom_quote_D where " & TRIM(cmbSearchField.selecteditem.value) & " like '%" & trim(txtSearch.text) & "%' ) and App2_By is not null  and BOM_Quote_Status = '" & trim(cmbStatus.selecteditem.value) & "' order by BOM_QUOTE_NO desc"
                                            ...
                                            <td valign="top" nowrap="nowrap" align="left" width="100%">
                                                ...
                                                <p>
                                                    <asp:DataGrid id="GridControl1" runat="server" width="100%" OnSelectedIndexChanged="GridControl1_SelectedIndexChanged" PagerStyle-PrevPageText="Prev" PagerStyle-NextPageText="Next" PageSize="20" AllowPaging="True" BorderColor="Black" GridLines="Vertical" cellpadding="4" ShowFooter="True" AutoGenerateColumns="False" OnPageIndexChanged="OurPager" PagerStyle-HorizontalAligh="Right" OnSortCommand="SortGrid" OnItemDataBound="FormatRow">
                                                        ...
                                                        <Columns>
                                                            <asp:HyperLinkColumn Text="View" DataNavigateUrlField="seq_no" DataNavigateUrlFormatString="BOMQuoteWorkSheetApp3.aspx?ID={0}"></asp:HyperLinkColumn>
