...
<script runat="server">
    ...
    Sub Page_Load(ByVal sender As Object, ByVal e As EventArgs)
        ...
        Sub ProcLoadGridData()
            ...
            ReqCOM.ExecuteNonQuery("Update FECN_M set Sort_Seq = 2 where App6_By is null and fecn_status = 'PENDING APPROVAL'")
    
    
            if trim(cmbSearch.selecteditem.value) = "FECN_No" then
                if trim(cmbFECNStatus.selecteditem.value) = "ALL" then StrSql = "SELECT * FROM FECN_M where App5_date is not null and App4_date is not null and App3_date is not null and App2_Date is not null and App1_date is not null AND " & trim(cmbSearch.selecteditem.value) & " like '%" & trim(txtSearch.text) & "%' ORDER BY Sort_Seq,FECN_No DESC"
                if trim(cmbFECNStatus.selecteditem.value) <> "ALL" then StrSql = "SELECT * FROM FECN_M where App5_date is not null and App4_date is not null and App3_date is not null and App2_Date is not null and App1_date is not null AND " & trim(cmbSearch.selecteditem.value) & " like '%" & trim(txtSearch.text) & "%' AND FECN_STATUS = '" & TRIM(cmbFECNStatus.selecteditem.value) & "' ORDER BY Sort_Seq,FEcn_No DESC"
            end if
            ...
            if trim(cmbSearch.selecteditem.value) = "PART_NO" then
                if trim(cmbFECNStatus.selecteditem.value) =  "ALL" then StrSql = "SELECT * FROM FECN_M where fecn_no in (Select Fecn_No from fecn_d where App5_date is not null and App4_date is not null and App3_date is not null and App2_Date is not null and App1_date is not null AND main_part_b4 like '%" & trim(txtSearch.text) & "%' or main_part like '%" & trim(txtSearch.text) & "%' or alt_part_b4 like '%" & trim(txtSearch.text) & "%' or alt_part like '%" & trim(txtSearch.text) & "%') ORDER BY Sort_Seq,FEcn_No DESC"
                if trim(cmbFECNStatus.selecteditem.value) <> "ALL" then StrSql = "SELECT * FROM FECN_M where fecn_no in (Select Fecn_No from fecn_d where App5_date is not null and App4_date is not null and App3_date is not null and App2_Date is not null and App1_date is not null AND main_part_b4 like '%" & trim(txtSearch.text) & "%' or main_part like '%" & trim(txtSearch.text) & "%' or alt_part_b4 like '%" & trim(txtSearch.text) & "%' or alt_part like '%" & trim(txtSearch.text) & "%') AND FECN_STATUS = '" & TRIM(cmbFECNStatus.selecteditem.value) & "' ORDER BY Sort_Seq,FEcn_No DESC"
            end if
    
            if trim(cmbSearch.selecteditem.value) = "LOT_NO" then
                if trim(cmbFECNStatus.selecteditem.value) =  "ALL" then StrSql = "SELECT * FROM FECN_M where App5_date is not null and App4_date is not null and App3_date is not null and App2_Date is not null and App1_date is not null AND fecn_no in (Select Fecn_No from fecn_d where lot_no like '%" & trim(txtSearch.text) & "%') ORDER BY Sort_Seq,FEcn_No DESC"
                if trim(cmbFECNStatus.selecteditem.value) <> "ALL" then StrSql = "SELECT * FROM FECN_M where App5_date is not null and App4_date is not null and App3_date is not null and App2_Date is not null and App1_date is not null AND fecn_no in (Select Fecn_No from fecn_d where lot_no like '%" & trim(txtSearch.text) & "%') AND FECN_STATUS = '" & TRIM(cmbFECNStatus.selecteditem.value) & "' ORDER BY Sort_Seq,FEcn_No DESC"
            end if
       ...
       Protected Sub FormatRow(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.DataGridItemEventArgs)
        ...
        If e.Item.ItemType = ListItemType.Item Or e.Item.ItemType = ListItemType.AlternatingItem Then
            ...
            if trim(App6Date.text) = "" then e.Item.CssClass = "PartSource"
            if trim(ucase(Status.text)) = "REJECTED" then
                if trim(NewFECNNo.text) = "" then e.Item.CssClass = "PartSource"
            End if
    
            if trim(App6Date.text) = "" and trim(ucase(Status.text)) <> "REJECTED" and trim(ucase(Urgent.text)) = "Y" then e.Item.CssClass = "Urgent"
        End if
                            ...
                            <p align="center">
                                <table style="HEIGHT: 7px" width="100%" border="1">
                                    <tbody>
                                        <tr>
                                            <td>
                                                <div align="center"><asp:Label id="Label1" runat="server" cssclass="OutputText">SEARCH</asp:Label>&nbsp;<asp:TextBox id="txtSearch" runat="server" Width="127px" CssClass="OutputText"></asp:TextBox>
                                                    &nbsp;<asp:Label id="Label3" runat="server" cssclass="OutputText">BY</asp:Label>&nbsp;<asp:DropDownList id="cmbSearch" runat="server" Width="137px" CssClass="OutputText">
                                                    ...
                                                    &nbsp;<asp:Label id="Label4" runat="server" cssclass="OutputText">SHOW</asp:Label>&nbsp;<asp:DropDownList id="cmbFECNStatus" runat="server" Width="178px" CssClass="OutputText">
                                                ...
                                                <p>
                                                    &nbsp;<asp:DataGrid id="GridControl1" runat="server" width="100%" OnSortCommand="SortGrid" AutoGenerateColumns="False" ShowFooter="True" cellpadding="4" GridLines="Vertical" BorderColor="Black" AllowPaging="True" OnPageIndexChanged="OurPager" PageSize="20" PagerStyle-NextPageText="Next" PagerStyle-PrevPageText="Prev" PagerStyle-HorizontalAligh="Right" OnSelectedIndexChanged="GridControl1_SelectedIndexChanged" OnItemDataBound="FormatRow">
                                                        ...
                                                        <Columns>
                                                            ...
                                                            <asp:TemplateColumn Visible="False">
                                                                <ItemTemplate>
                                                                    <asp:Label id="NewFECNNo" visible="false" width="" runat="server" text='<%# DataBinder.Eval(Container.DataItem, "New_FECN_No") %>' /> 
                                                                </ItemTemplate>
                                                            </asp:TemplateColumn>
                                                            <asp:TemplateColumn Visible="False">
                                                                <ItemTemplate>
