...
<script runat="server">
    ...
    Sub ProcLoadGridData()
        ...
        if trim(cmbStatus.selecteditem.value) = "ALL" then
            if trim(cmbSearchField.selecteditem.value) = "BOM_QUOTE_NO" then StrSql = "SELECT * FROM BOM_QUOTE_M WHERE " & TRIM(cmbSearchField.selecteditem.value) & " like '%" & trim(txtSearch.text) & "%' and submit_by is not null order by BOM_QUOTE_NO desc"
            if trim(cmbSearchField.selecteditem.value) = "MODEL_NO" then StrSql = "SELECT * FROM BOM_QUOTE_M WHERE " & TRIM(cmbSearchField.selecteditem.value) & " like '%" & trim(txtSearch.text) & "%' and submit_by is not null  order by BOM_QUOTE_NO desc"
            if trim(cmbSearchField.selecteditem.value) = "MODEL_DESC" then StrSql = "SELECT * FROM BOM_QUOTE_M WHERE " & TRIM(cmbSearchField.selecteditem.value) & " like '%" & trim(txtSearch.text) & "%' and submit_by is not null  order by BOM_QUOTE_NO desc"
            if trim(cmbSearchField.selecteditem.value) = "PART_SPEC" then StrSql = "SELECT * FROM BOM_QUOTE_M WHERE bom_quote_no in (Select BOM_Quote_No from bom_quote_D where " & TRIM(cmbSearchField.selecteditem.value) & " like '%" & trim(txtSearch.text) & "%' ) and submit_by is not null  order by BOM_QUOTE_NO desc"
            if trim(cmbSearchField.selecteditem.value) = "PART_DESC" then StrSql = "SELECT * FROM BOM_QUOTE_M WHERE bom_quote_no in (Select BOM_Quote_No from bom_quote_D where " & TRIM(cmbSearchField.selecteditem.value) & " like '%" & trim(txtSearch.text) & "%' ) and submit_by is not null  order by BOM_QUOTE_NO desc"
            if trim(cmbSearchField.selecteditem.value) = "CUST_part_no" then StrSql = "SELECT * FROM BOM_QUOTE_M WHERE bom_quote_no in (Select BOM_Quote_No from bom_quote_D where " & TRIM(cmbSearchField.selecteditem.value) & " like '%" & trim(txtSearch.text) & "%' ) and submit_by is not null  order by BOM_QUOTE_NO desc"
            if trim(cmbSearchField.selecteditem.value) = "PART_NO" then StrSql = "SELECT * FROM BOM_QUOTE_M WHERE bom_quote_no in (Select BOM_Quote_No from bom_quote_D where " & TRIM(cmbSearchField.selecteditem.value) & " like '%" & trim(txtSearch.text) & "%' ) and submit_by is not null  order by BOM_QUOTE_NO desc"
        elseif trim(cmbStatus.selecteditem.value) <> "ALL" then
            if trim(cmbSearchField.selecteditem.value) = "BOM_QUOTE_NO" then StrSql = "SELECT * FROM BOM_QUOTE_M WHERE " & TRIM(cmbSearchField.selecteditem.value) & " like '%" & trim(txtSearch.text) & "%' and BOM_Quote_Status = '" & trim(cmbStatus.selecteditem.value) & "' and submit_by is not null  order by BOM_QUOTE_NO desc"
            if trim(cmbSearchField.selecteditem.value) = "MODEL_NO" then StrSql = "SELECT * FROM BOM_QUOTE_M WHERE " & TRIM(cmbSearchField.selecteditem.value) & " like '%" & trim(txtSearch.text) & "%' and BOM_Quote_Status = '" & trim(cmbStatus.selecteditem.value) & "' and submit_by is not null  order by BOM_QUOTE_NO desc"
            if trim(cmbSearchField.selecteditem.value) = "MODEL_DESC" then StrSql = "SELECT * FROM BOM_QUOTE_M WHERE " & TRIM(cmbSearchField.selecteditem.value) & " like '%" & trim(txtSearch.text) & "%' and BOM_Quote_Status = '" & trim(cmbStatus.selecteditem.value) & "' and submit_by is not null  order by BOM_QUOTE_NO desc"
            if trim(cmbSearchField.selecteditem.value) = "PART_SPEC" then StrSql = "SELECT * FROM BOM_QUOTE_M WHERE bom_quote_no in (Select BOM_Quote_No from bom_quote_D where " & TRIM(cmbSearchField.selecteditem.value) & " like '%" & trim(txtSearch.text) & "%' ) and submit_by is not null  and BOM_Quote_Status = '" & trim(cmbStatus.selecteditem.value) & "' order by BOM_QUOTE_NO desc"
            ...
            <table style="HEIGHT: 11px" cellspacing="0" cellpadding="0" width="100%">
                <tbody>
                    ...
                    <tr>
                        <td>
                            <p align="center">
                                <asp:Label id="Label1" runat="server" cssclass="FormDesc" width="100%">BOM QUOTATION
                                LIST</asp:Label>
                            </p>
                            <p align="center">
                                                            ...
                                                            <tr>
                                                                <td>
                                                                    <p align="center">
                                                                        ...
                                                                        <asp:TextBox id="txtSearch" runat="server" CssClass="OutputText" Width="164px" Height="19px"></asp:TextBox>
                                                                        &nbsp; <asp:Label id="Label2" runat="server" cssclass="OutputText">by</asp:Label>&nbsp; 
                                                                        <asp:DropDownList id="cmbSearchField" runat="server" CssClass="OutputText" Width="161px" Height="19px" OnSelectedIndexChanged="cmbSearchField_SelectedIndexChanged">
                                                                        ...
                                                                        &nbsp;&nbsp;&nbsp;&nbsp;<asp:DropDownList id="cmbStatus" runat="server" CssClass="OutputText" Width="166px" Height="19px" OnSelectedIndexChanged="cmbSearchField_SelectedIndexChanged">
                                                                        ...
                                                                        &nbsp;&nbsp;&nbsp; 
                                                                        <asp:Button id="Button1" onclick="Button1_Click_1" runat="server" CssClass="OutputText" Width="43px" Text="GO"></asp:Button>
                                                                    </p>
                                                                </td>
                                                            </tr>
                                                        </tbody>
