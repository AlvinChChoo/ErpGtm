...
<script runat="server">
    ...
    Sub Page_Load(ByVal sender As Object, ByVal e As EventArgs)
            if page.isPostBack = false then
                ...
                do while rs.read
                    ...
                    if isdbnull(rs("App2_Date")) = true then
                        cmdsubmit.enabled = true
                        rbApprove.visible = true
                        rbReject.visible = true
                        txtRem.visible = true
                        Label11.visible = true
                    elseif isdbnull(rs("App2_Date")) = false then
    ...
    Sub cmdBack_Click(sender As Object, e As EventArgs)
        Response.redirect("KitLotApp1.aspx")
    End Sub
    ...
    Sub cmdSubmit_Click(sender As Object, e As EventArgs)
        ...
        if rbApprove.checked = true then
            ReqCOM.ExecuteNonQuery("Update Kit_Lot set Kit_Lot_Status = 'APPROVED',APP2_STATUS='Y',App2_By = '" & trim(Request.cookies("U_ID").value) & "', App2_Date = '" & cdate(now) & "' where Kit_Lot_No = '" & trim(lblKitLotNo.text) & "';")
        end if
    
        if rbReject.checked = true then
            ReqCOM.ExecuteNonQuery("Update Kit_Lot set Kit_Lot_Status = 'REJECTED',APP2_STATUS = 'N',APP2_REM = '" & TRIM(REPLACE(TXTrEM.TEXT,"'","`")) & "',App2_By = '" & trim(Request.cookies("U_ID").value) & "', App2_Date = '" & cdate(now) & "' where Kit_Lot_No = '" & trim(lblKitLotNo.text) & "';")
        end if
        redirectPage("KitLotApp1Det.aspx?ID=" & Request.params("ID"))
    End Sub
                                                ...
                                                <p>
                                                    <table id="table" style="FONT-SIZE: xx-small; BORDER-LEFT-COLOR: black; BORDER-BOTTOM-COLOR: black; WIDTH: 100%; BORDER-TOP-COLOR: black; FONT-FAMILY: Verdana; BORDER-COLLAPSE: collapse; BORDER-RIGHT-COLOR: black" width="100%" border="1">
                                                        <tbody>
                                                            <tr>
                                                                <td width="25%">
                                                                    <asp:Label id="Label11" runat="server" cssclass="OutputText">Remarks</asp:Label></td>
                                                                <td width="55%">
