...
<script runat="server">
    ...
    Sub ProcLoadGridData()
        ...
        if cmbSearch.selecteditem.value = "UPA_NO" then StrSql = "SELECT * FROM UPAS_M WHERE UPAS_NO LIKE '%" & txtSearch.Text & "%' and acc1_by is not null and UPAS_Status like '%" & trim(cmbUPAStatus.selecteditem.value) & "%' ORDER BY upas_no desc"
        if cmbSearch.selecteditem.value = "SUBMIT_BY" then StrSql = "SELECT * FROM UPAS_M WHERE SUBMIT_BY LIKE '%" & TRIM(txtSearch.text) & "%' AND acc1_by is not null and UPAS_Status like '%" & trim(cmbUPAStatus.selecteditem.value) & "%' ORDER BY upas_no desc"
        if cmbSearch.selecteditem.value = "VEN_CODE" then StrSql = "SELECT * FROM UPAS_M WHERE UPAS_NO in (SELECT UPAS_NO FROM UPAS_D WHERE VEN_CODE IN(Select VEN_CODE from VENDOR where VEN_CODE + VEN_NAME like '%" & trim(txtSearch.text) & "%')) AND acc1_by is not null and UPAS_Status like '%" & trim(cmbUPAStatus.selecteditem.value) & "%' ORDER BY upas_no desc"

        if cmbSearch.selecteditem.value = "PART_NO" then
            if txtSearch.text = "" then StrSql = "SELECT * FROM UPAS_M WHERE acc1_by is not null and upas_status <> 'REJECTED' and UPAS_Status like '%" & trim(cmbUPAStatus.selecteditem.value) & "%' ORDER BY upas_no desc"
            if txtSearch.text <> "" then StrSql = "SELECT * FROM UPAS_M WHERE UPAS_NO in (Select UPAS_NO from UPAS_D where Part_No like '%" & trim(txtSearch.text) & "%') and acc1_by is not null and UPAS_Status like '%" & trim(cmbUPAStatus.selecteditem.value) & "%' ORDER BY upas_no desc"
        End if

        'StrSql = "SELECT * FROM UPAS_M WHERE UPAS_NO LIKE '%" & txtSearch.Text & "%' and acc1_by is not null and UPAS_Status like '%" & trim(cmbUPAStatus.selecteditem.value) & "%' ORDER BY upas_no desc"

        'Dim myConnection As SqlConnection = New SqlConnection(ConfigurationSettings.AppSettings("ConnectionString"))
'        response.write(ConfigurationSettings.AppSettings("ConnectionString"))
        ...
        Dim resExePagedDataSet as Dataset = ReqExePagedDataSet.ExePagedDataSet(StrSql,"UPAs_m")
        GridControl1.DataSource=resExePagedDataSet.Tables("UPAs_m").DefaultView
        GridControl1.DataBind()
    end Sub
    ...
    Protected Sub FormatRow(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.DataGridItemEventArgs)
        ...
        If e.Item.ItemType = ListItemType.Item Or e.Item.ItemType = ListItemType.AlternatingItem Then
            Dim Acc1App As Label = CType(e.Item.FindControl("Acc1App"), Label)
            Dim Acc2App As Label = CType(e.Item.FindControl("Acc2App"), Label)
            ...
            Dim ACC1Date As Label = CType(e.Item.FindControl("ACC1Date"), Label)
            Dim ACC2Date As Label = CType(e.Item.FindControl("ACC2Date"), Label)
            ...
            if trim(ACC1Date.text) <> "" then ACC1Date.text = format(cdate(ACC1Date.text),"dd/MMM/yy")
            if trim(ACC2Date.text) <> "" then ACC2Date.text = format(cdate(ACC2Date.text),"dd/MMM/yy")
            ...
            if trim(Acc2App.text) = "" then
                if trim(Status.text) <> "REJECTED" then
                                        ...
                                        <tr>
                                            <td>
                                                <div align="center"><asp:Label id="Label3" runat="server" cssclass="OutputText">SEARCH</asp:Label>&nbsp;&nbsp;
                                                    ...
                                                    &nbsp;&nbsp; <asp:Label id="Label2" runat="server" cssclass="OutputText">BY</asp:Label>&nbsp;&nbsp;&nbsp;<asp:DropDownList id="cmbSearch" runat="server" CssClass="OutputText" Width="143px">
                                                    ...
                                                    &nbsp;&nbsp;&nbsp;&nbsp;<asp:Label id="Label7" runat="server" cssclass="OutputText">Show </asp:Label>&nbsp;&nbsp;<asp:DropDownList id="cmbUPAStatus" runat="server" CssClass="OutputText" Width="172px">
                                                    ...
                                                    &nbsp;&nbsp;
                                                    <asp:Button id="Button3" onclick="Button3_Click" runat="server" CssClass="OutputText" Width="65px" Text="GO"></asp:Button>
                                                </div>
                                            </td>
                                        </tr>
                                    </tbody>
                                ...
                                <table style="HEIGHT: 10px" cellspacing="0" cellpadding="0" width="94%">
                                    <tbody>
                                        <tr>
                                            <td>
                                                <p>
                                                    <asp:DataGrid id="GridControl1" runat="server" width="100%" PagerStyle-HorizontalAligh="Right" PagerStyle-PrevPageText="Prev" PagerStyle-NextPageText="Next" PageSize="20" OnPageIndexChanged="OurPager" OnItemDataBound="FormatRow" Font-Name="Verdana" AutoGenerateColumns="False" ShowFooter="True" cellpadding="4" GridLines="Vertical" BorderColor="Black" AllowPaging="True" OnSelectedIndexChanged="GridControl1_SelectedIndexChanged">
                                                        <FooterStyle cssclass="GridFooter"></FooterStyle>
                                                        <AlternatingItemStyle cssclass="GridItemAlt"></AlternatingItemStyle>
                                                        <ItemStyle cssclass="GridItem"></ItemStyle>
                                                        <HeaderStyle bordercolor="White" cssclass="GridHeader"></HeaderStyle>
                                                        <Columns>
                                                            <asp:HyperLinkColumn Text="View" DataNavigateUrlField="Seq_No" DataNavigateUrlFormatString="UPAAcc2AppDet.aspx?ID={0}"></asp:HyperLinkColumn>
                                                            ...
                                                            <asp:TemplateColumn HeaderText="A/C 1">
                                                                <ItemTemplate>
                                                                    <asp:Label id="Acc1App" runat="server" text='<%# DataBinder.Eval(Container.DataItem, "ACC1_BY") %>' /> - <asp:Label id="Acc1Date" runat="server" text='<%# DataBinder.Eval(Container.DataItem, "ACC1_DATE") %>' />
                                                                </ItemTemplate>
                                                            </asp:TemplateColumn>
                                                            <asp:TemplateColumn HeaderText="A/C 2">
                                                                <ItemTemplate>
                                                                    <asp:Label id="Acc2App" runat="server" text='<%# DataBinder.Eval(Container.DataItem, "ACC2_BY") %>' /> - <asp:Label id="Acc2Date" runat="server" text='<%# DataBinder.Eval(Container.DataItem, "ACC2_DATE") %>' />
                                                                </ItemTemplate>
                                                            </asp:TemplateColumn>
