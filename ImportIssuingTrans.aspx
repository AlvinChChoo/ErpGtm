...
<script runat="server">
    ...
    Sub ImportTextData()
        ...
        Dim Str,LineIn,LotNo,MainPart,PartNo,PLevel,ModelNo as string
        Dim ReqQty,QtyIssued,QtyIR,QtyScrap,QthOtherScrap,QtyReturn,BalToIssue as long
        Dim PUsage as decimal
        Dim i,TotalLen,LeftStr as integer

        try
            ReqCOM.ExecuteNonQuery("Truncate Table ExpISS")
            oRead = oFile.OpenText(Mappath("") + “\Data Import\EXPISS.txt”)
            While oRead.Peek <> -1
                ...
                Str = Replace(Str,"'","`")
                    ...
                    For i = 1 to 14
                        if i <> 14 then
                                ...
                                if i = 1 then
                                    LotNo = trim(left(Str,LeftStr))
                                    LotNo = replace(LotNo,"""","")
                                Elseif i = 2 then
                                    ModelNo = trim(left(Str,LeftStr))
                                    ModelNo = replace(ModelNo,"""","")
                                Elseif i = 3 then
                                    MainPart = trim(left(Str,LeftStr))
                                    MainPart = replace(MainPart,"""","")
                                Elseif i = 4 then
                                    PartNo = trim(left(Str,LeftStr))
                                    PartNo = replace(PartNo,"""","")
                                Elseif i = 5 then
                                    PLevel = trim(left(Str,LeftStr))
                                    PLevel = replace(PLevel,"""","")




                                Elseif i = 5 then
                                    'PUsage = trim(left(Str,LeftStr))
            ...
            next i
                ...
                ReqCOM.executeNonQuery("Insert into expiss(Lot_No,Main_Part,Part_No,P_Level,Model_No) select '" & trim(LotNo) & "','" & trim(MainPart) & "','" & trim(PartNo) & "','" & trim(PLevel) & "','" & trim(ModelNo) & "';")
