...
<script runat="server">
    ...
    Sub Page_Load(ByVal sender As Object, ByVal e As EventArgs)
        if page.isPostBack = false then
            ...
            Do while RsApproval.read
                ...
                if isdbnull(RsApproval("App3_By")) = false then
    ...
    Sub cmdBack_Click(sender As Object, e As EventArgs)
        Response.redirect("PRApp3.aspx")
    End Sub

    Sub cmdSubmit_Click(sender As Object, e As EventArgs)
        if page.isvalid = true then
            Dim ReqCOM as ERP_GTM.ERP_GTM = new ERP_GTM.ERP_GTM
            Dim PRType as string = ReqCOM.GetFieldVal("select PR_TYPE from pr1_m where pr_no = '" & trim(lblPRNo.text) & "';","PR_TYPE")
            Dim MSender,MReceiver,CC,StrSql as string
            ...
            if rbApprove.checked = true then
                if PrType = "SF" then
                    ...
                    'MReceiver = ReqCOM.GetFieldVal("Select Email from User_Profile where U_ID in (Select U_ID from authority where Module_Name = 'PR' and app_type = 'APP4')","Email")
                    'GenerateMail(MSender,MReceiver,CC)
                    ReqCOM.ExecuteNonQuery("Update PR1_M set App3_By = '" & trim(request.cookies("U_ID").value) & "',App3_Date = '" & now & "',App3_Rem = '" & trim(txtRem.text) & "',App3_Status='Y',PR_Status = 'PENDING P/O GEN.' where PR_No = '" & trim(lblPRNo.text) & "';")
                ElseIf PrType = "SO" then
                    if rbApprove.checked = true then
                        MSender = trim(Request.cookies("U_ID").value)
                        MReceiver = ReqCOM.GetFieldVal("Select App1_By from PR1_M where PR_No = '" & trim(lblPRNo.text) & "';","App1_By")
                        GeneratePendingEmailList(MSender,MReceiver,trim(lblPRNo.text),"Y")
                    ...
                    'GenerateMailFirmSO(MSender,MReceiver,CC)
                    ReqCOM.ExecuteNonQuery("Update PR1_M set App3_By = '" & trim(request.cookies("U_ID").value) & "',App3_Date = '" & now & "',App3_Rem = '" & trim(txtRem.text) & "',App3_Status='Y',App4_By = 'N/A',App4_Date = '" & now & "',App4_Status = 'Y' ,PR_Status = 'PENDING P/O GEN.' where PR_No = '" & trim(lblPRNo.text) & "';")
                End if

                ShowAlert ("SR sumbitted for further approval.")
            elseif rbReject.checked = true then
                ReqCOM.ExecuteNonQuery("Update PR1_M set App3_By = '" & trim(request.cookies("U_ID").value) & "',App3_Date = '" & now & "',App3_Rem = '" & trim(txtRem.text) & "',App3_Status='N',pr_status = 'REJECTED' where PR_No = '" & trim(lblPRNo.text) & "';")
                ShowAlert ("Selected SR has been rejected.")
            end if
            redirectPage("PRApp3Det.aspx?ID=" & Request.params("ID"))
        End if
    End Sub

    Sub GeneratePendingEmailList(Sender as string, Receiver as string,DOcNo as string,SSERStat as string)
        Dim ReqCOM as ERP_GTM.ERP_GTM = new ERP_GTM.ERP_GTM
        Dim FromEmail,ToEmail,EmailSubject,EmailContent as string
        Dim PRType as string = ReqCOM.GetFieldVal("select PR_TYPE from pr1_m where pr_no = '" & trim(lblPRNo.text) & "';","PR_TYPE")


        if trim(SSERStat) = "Y" then
            EmailContent = "Dear " & trim(Receiver) & vblf & vblf
            EmailContent = EmailContent + "There is a New P/R pending for your P/O explosion (P/R # : " & trim(DOcNo) & ")." & vblf & vblf

            if trim(PRType) = "SO" then EmailContent = EmailContent + "Click on http://gtekapp/erp/signin.aspx?ReturnURL=PRApp5Det.aspx?ID=" & ReqCOM.GetFieldVal("Select Seq_No from PR1_M where PR_No = '" & trim(DOcNo) & "';","Seq_No") & " to view the details." & vblf & vblf
            if trim(PRType) = "SF" then EmailContent = EmailContent + "Click on http://gtekapp/erp/signin.aspx?ReturnURL=PRApp4Det.aspx?ID=" & ReqCOM.GetFieldVal("Select Seq_No from PR1_M where PR_No = '" & trim(DOcNo) & "';","Seq_No") & " to view the details." & vblf & vblf

            EmailContent = EmailContent + "Regards," & vblf
            EmailContent = EmailContent + trim(Sender)

            if trim(PRType) = "SO" then EmailSubject = "Pending P/O Explosion : " & DOcNo
            if trim(PRType) = "SF" then EmailSubject = "P/R Approval : " & DOcNo
    ...
    Sub GenerateMailFirmSO(Sender as string, Receiver as string,CC as string)
