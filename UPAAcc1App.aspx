...
<script runat="server">
    ...
    Sub ProcLoadGridData()
        ...
        if cmbSearch.selecteditem.value = "UPA_NO" then StrSql = "SELECT * FROM UPAS_M WHERE UPAS_NO LIKE '%" & txtSearch.Text & "%' and PURC_BY is not null and upas_status like '%" & trim(cmbUPAStatus.selecteditem.value) & "%' ORDER BY upas_no desc"
        if cmbSearch.selecteditem.value = "SUBMIT_BY" then StrSql = "SELECT * FROM UPAS_M WHERE SUBMIT_BY LIKE '%" & TRIM(txtSearch.text) & "%' AND PURC_BY is not null and upas_status like '%" & trim(cmbUPAStatus.selecteditem.value) & "%' ORDER BY upas_no desc"
        if cmbSearch.selecteditem.value = "VEN_CODE" then StrSql = "SELECT * FROM UPAS_M WHERE UPAS_NO in (SELECT UPAS_NO FROM UPAS_D WHERE VEN_CODE IN(Select VEN_CODE from VENDOR where VEN_CODE + VEN_NAME like '%" & trim(txtSearch.text) & "%')) AND PURC_BY is not null and upas_status like '%" & trim(cmbUPAStatus.selecteditem.value) & "%' ORDER BY upas_no desc"
    
        if cmbSearch.selecteditem.value = "PART_NO" then
            if txtSearch.text = "" then StrSql = "SELECT * FROM UPAS_M WHERE PURC_BY is not null and upas_status like '%" & trim(cmbUPAStatus.selecteditem.value) & "%' ORDER BY upas_no desc"
            if txtSearch.text <> "" then StrSql = "SELECT * FROM UPAS_M WHERE UPAS_NO in (Select UPAS_NO from UPAS_D where Part_No like '%" & trim(txtSearch.text) & "%') and PURC_BY is not null and upas_status like '%" & trim(cmbUPAStatus.selecteditem.value) & "%' ORDER BY upas_no desc"
        End if
                                        ...
                                        <tr>
                                            <td>
                                                <div align="center"><asp:Label id="Label3" runat="server" cssclass="OutputText">SEARCH</asp:Label>&nbsp;&nbsp; 
                                                    ...
                                                    <asp:DropDownList id="cmbUPAStatus" runat="server" CssClass="OutputText" Width="172px">
                                                    ...
                                                    &nbsp;<asp:Button id="Button3" onclick="Button3_Click" runat="server" CssClass="OutputText" Width="78px" Text="GO"></asp:Button>
                                                </div>
                                            </td>
                                        </tr>
                                    </tbody>
                                            ...
                                            <td>
                                                <p>
                                                    <asp:DataGrid id="GridControl1" runat="server" width="100%" PagerStyle-HorizontalAligh="Right" PagerStyle-PrevPageText="Prev" PagerStyle-NextPageText="Next" PageSize="20" OnPageIndexChanged="OurPager" OnItemDataBound="FormatRow" Font-Name="Verdana" AutoGenerateColumns="False" ShowFooter="True" cellpadding="4" GridLines="Vertical" BorderColor="Black" AllowPaging="True" OnSelectedIndexChanged="GridControl1_SelectedIndexChanged">
                                                        ...
                                                        <Columns>
                                                            <asp:HyperLinkColumn Text="View" DataNavigateUrlField="Seq_No" DataNavigateUrlFormatString="UPAAcc1AppDet.aspx?ID={0}"></asp:HyperLinkColumn>
