...
<script runat="server">
    ...
    Sub ProcLoadGridData()
        ...
        if cmbSearch.selecteditem.value = "SSER_NO" then
            StrSql = "Select * from sser_m where SSER_NO like '%" & trim(txtSearch.text) & "%' and (sser_stat = 'APPROVED' OR sser_stat = 'REJECTED') order by seq_no desc"
        elseif cmbSearch.selecteditem.value = "PART_NO" then
            StrSql = "Select * from sser_m where Part_No_From + Part_No_To like '%" & trim(txtSearch.text) & "%' and (sser_stat = 'APPROVED' OR sser_stat = 'REJECTED') order by seq_no desc"
        end if
    ...
    Sub ShowSSER(sender as Object,e as DataGridCommandEventArgs)
        ...
        ShowReport("PopupSSER.aspx?RptName=SSER&SSERNo=" & trim(SSERNo.text))
    End sub
                                        ...
                                        <tr>
                                            <td>
                                                <p align="center">
                                                    ...
                                                    <asp:DropDownList id="cmbSearch" runat="server" Width="238px" CssClass="OutputText" Height="19px">
