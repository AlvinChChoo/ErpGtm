...
<script runat="server">
    ...
    Sub Page_Load(ByVal sender As Object, ByVal e As EventArgs)
        cmdDelete.attributes.add("onClick","javascript:if(confirm('This action will remove the selected attachment from this Supplier Information.\nYou will not be able to undo the changes made.\nAre you sure to continue ?')==false) return false;")
        if page.ispostback = false then
            Dim ReqCOM as Erp_Gtm.Erp_Gtm = new Erp_Gtm.Erp_Gtm
            lblRefNo.text = reqCOM.GetFieldVal("Select top 1 Ref_No from Vendor_Info where Seq_No = " & request.params("ID") & ";","Ref_No")
            procLoadGridData()
        end if
    End Sub
    ...
    Sub ProcLoadGridData()
        ...
        dtgModelPic.DataSource=resExePagedDataSet.Tables("VENDOR_INFO_ATT").DefaultView
    ...
    Sub cmdDelete_Click(sender As Object, e As EventArgs)
        ...
        For i = 0 To dtgModelPic.Items.Count - 1
            ...
            If remove.Checked = True Then ReqExecuteNoNQuery.ExecuteNonQuery("Delete from VENDOR_INFO_ATT where Seq_No = " & SeqNo.text & ";")
        Next
        procLoadGridData()
    End Sub
    ...
    Sub Button1_Click(sender As Object, e As EventArgs)
        if page.isvalid = true then
            ...
            StrSql = "Insert into VENDOR_INFO_ATT(File_Desc,ref_no,File_Size) "
            StrSql = StrSql + "Select '" & trim(txtfileDesc.text) & "','" & trim(LBLrEFnO.text) & "'," & FileLength & ";"
            ReqCOM.ExecuteNonQuery(StrSql)
    
            SeqNo = ReqCOM.GetFieldVal("Select top 1 Seq_No from VENDOR_INFO_ATT where REF_NO = '" & trim(lblRefNo.text) & "' and File_Desc = '" & trim(txtFileDesc.text) & "' order by seq_no desc","Seq_No")
            FileName1 = SeqNo & "." & FileExt
    
            StrSql = "Update VENDOR_INFO_ATT set file_name = '" & trim(FileName1) & "' where seq_no = " & SeqNo & ";"
            ReqCOM.ExecuteNonQuery(StrSql)
    
            fileControl.PostedFile.SaveAs((Mappath("") + "\VendorInfoAttachment\" + FileName1))
            Response.redirect("PopupSupplierInfoAtt.aspx?ID=" & Request.params("ID"))
        end if
    End Sub
            ...
            <table style="HEIGHT: 4px" cellspacing="0" cellpadding="0" width="100%">
                <tbody>
                    <tr>
                        <td>
                            <p align="center">
                                <asp:Label id="Label1" runat="server" width="100%" cssclass="FormDesc">SUPPLIER INFORMTION
                                ATTACHMENT</asp:Label>
                            </p>
                            <p align="center">
                                <asp:RequiredFieldValidator id="RequiredFieldValidator1" runat="server" ForeColor=" " EnableClientScript="False" Display="Dynamic" ControlToValidate="txtFileDesc" ErrorMessage="You don't seem to have supplied a valid file descriptiuon." CssClass="ErrorText" Width="100%"></asp:RequiredFieldValidator>
                                                        ...
                                                        <tbody>
                                                            <tr>
                                                                <td width="25%" bgcolor="silver">
                                                                    <asp:Label id="Label2" runat="server" cssclass="LabelNormal">Ref No</asp:Label></td>
                                                                <td>
                                                                    <asp:Label id="lblRefNo" runat="server" width="359px" cssclass="OutputText">Label</asp:Label></td>
                                                            </tr>
                                                            <tr>
                                                                <td bgcolor="silver">
                                                            ...
                                                            <tr>
                                                                <td colspan="2">
                                                                    <p align="center">
                                                                        <asp:Button id="Button1" onclick="Button1_Click" runat="server" CssClass="OutputText" Text="Attach File to supplier"></asp:Button>
