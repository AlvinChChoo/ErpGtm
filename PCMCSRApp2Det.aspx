...
<script runat="server">
    ...
    Sub Page_Load(ByVal sender As Object, ByVal e As EventArgs)
        ...
        End Sub
         ...
         Sub cmdBack_Click(sender As Object, e As EventArgs)
             Response.redirect("PCMCSRApp2.aspx")
         End Sub
    ...
    Sub cmdApprove_Click(sender As Object, e As EventArgs)
        ...
        if rbApprove.checked = true then
            ReqCOM.ExecuteNonQuery("Update SR_M set App2_By = '" & trim(request.cookies("U_ID").value) & "',App2_Date = '" & now & "',App2_Rem = '" & trim(txtRem.text) & "',App2_Status='Y' where SR_No = '" & trim(lblSRNo.text) & "';")
            ...
            MReceiver = ReqCOM.GetFieldVal("Select EMail from User_Profile where U_ID in (Select U_ID from authority where app_type = 'APP3' and module_name = 'PCMCSR')","Email")
        ...
        elseif rbReject.checked = true then
            ReqCOM.ExecuteNonQuery("Update SR_M set App2_By = '" & trim(request.cookies("U_ID").value) & "',App2_Date = '" & now & "',App2_Rem = '" & trim(txtRem.text) & "',App2_Status='N',sr_status = 'REJECTED' where SR_No = '" & trim(lblSRNo.text) & "';")
            MReceiver = ReqCOM.GetFieldVal("Select EMail from User_Profile where U_ID in (Select App1_By from sr_m where sr_no = '" & trim(lblSRNo.text) & "')","Email")
            MSender = ReqCOM.GetFieldVal("Select EMail from User_Profile where U_ID = '" & trim(request.cookies("U_ID").value) & "';","Email")
            GenerateMail(MSender,MReceiver,CC,trim(lblSRNo.text),"N")
            ShowAlert ("Selected SR has been rejected.")
        end if
        redirectPage("PCMCSRApp2Det.aspx?ID=" & Request.params("ID"))
    End Sub
    ...
    Sub GenerateMail(Sender as string, Receiver as string,CC as string,DOcNo as string,SRStatus as string)
        ...
        if SRStatus = "Y" then
            ...
            StrMsg = StrMsg + "Click on http://gtekapp/erp/signin.aspx?ReturnURL=PCMCSRApp3Det.aspx?ID=" & ReqCOM.GetFieldVal("Select Seq_No from SR_M where SR_NO = '" & trim(DOcNo) & "';","Seq_No") & " to view the details."   & vblf & vblf
