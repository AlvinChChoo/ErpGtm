...
<script runat="server">
    ...
    Sub ProcLoadGridData()
        ...
        if cmbSearch.selecteditem.value = "SSER_NO" then
            StrSql = "Select * from sser_m where SSER_NO like '%" & trim(txtSearch.text) & "%' and ME_ENG_Date is not null and sser_stat <> 'REJECTED' order by seq_no desc"
        elseif cmbSearch.selecteditem.value = "PART_NO" then
            StrSql = "Select * from sser_m where Part_No_From + Part_No_To like '%" & trim(txtSearch.text) & "%' and ME_ENG_Date is not null and sser_stat <> 'REJECTED' order by seq_no desc"
        end if
    ...
    Sub ShowSSER(sender as Object,e as DataGridCommandEventArgs)
        ...
        Response.redirect("SSERMEHODDet.aspx?ID=" & SeqNo)
    End sub
