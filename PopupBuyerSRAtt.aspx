...
<script runat="server">
    ...
    Sub Page_Load(ByVal sender As Object, ByVal e As EventArgs)
        ...
        if page.ispostback = false then
            ...
            lblSRNo.text = reqCOM.GetFieldVal("Select top 1 SR_NO from BUYER_SR_m where Seq_No = " & request.params("ID") & ";","SR_NO")
            procLoadGridData()
        end if
    End Sub
    ...
    Sub ProcLoadGridData()
        ...
        dtgModelPic.DataSource=resExePagedDataSet.Tables("BUYER_SR_ATTACHMENT").DefaultView
    ...
    Sub cmdDelete_Click(sender As Object, e As EventArgs)
        ...
        For i = 0 To dtgModelPic.Items.Count - 1
            ...
            If remove.Checked = True Then ReqExecuteNoNQuery.ExecuteNonQuery("Delete from BUYER_SR_ATTACHMENT where Seq_No = " & SeqNo.text & ";")
        Next
        procLoadGridData()
    End Sub
    ...
    Sub Button1_Click(sender As Object, e As EventArgs)
        if page.isvalid = true then
            ...
            StrSql = "Insert into BUYER_SR_ATTACHMENT(File_Desc,SR_NO,File_Size) "
            StrSql = StrSql + "Select '" & trim(txtfileDesc.text) & "','" & trim(lblSRNo.text) & "'," & FileLength & ";"
            ReqCOM.ExecuteNonQuery(StrSql)
            SeqNo = ReqCOM.GetFieldVal("Select top 1 Seq_No from BUYER_SR_ATTACHMENT where SR_NO = '" & trim(lblSRNo.text) & "' and File_Desc = '" & trim(txtFileDesc.text) & "' order by seq_no desc","Seq_No")
            FileName1 = SeqNo & "." & FileExt
            StrSql = "Update BUYER_SR_ATTACHMENT set file_name = '" & trim(FileName1) & "' where seq_no = " & SeqNo & ";"
            ReqCOM.ExecuteNonQuery(StrSql)
            fileControl.PostedFile.SaveAs((Mappath("") + "\BuyerSRAttachment\" + FileName1))
            Response.redirect("PopupBuyerSRAtt.aspx?ID=" & Request.params("ID"))
        end if
    End Sub
            ...
            <table style="HEIGHT: 4px" cellspacing="0" cellpadding="0" width="100%">
                <tbody>
                    <tr>
                        <td>
                            <p align="center">
                                <asp:Label id="Label1" runat="server" cssclass="FormDesc" width="100%">SPECIAL REQUEST(BUYER)
                                                    ...
                                                    <table style="FONT-SIZE: xx-small; BORDER-LEFT-COLOR: white; BORDER-BOTTOM-COLOR: white; WIDTH: 100%; BORDER-TOP-COLOR: white; FONT-FAMILY: Verdana; BORDER-COLLAPSE: collapse; BORDER-RIGHT-COLOR: white" width="100%" border="1">
                                                        <tbody>
                                                            <tr>
                                                                ...
                                                                <td>
                                                                    <asp:Label id="lblSRNo" runat="server" cssclass="OutputText" width="359px">Label</asp:Label></td>
                                                            </tr>
                                                            <tr>
                                                                <td bgcolor="silver">
