    ...
    Sub Page_Load(ByVal sender As Object, ByVal e As EventArgs)
            ...
            End Sub
              ...
              Sub cmdBack_Click(sender As Object, e As EventArgs)
                  response.redirect("MRFApp1.aspx")
              End Sub
            ...
            Sub cmdApproved_Click(sender As Object, e As EventArgs)
                ...
                if rbApprove.checked = true then
                    ReqCOM.ExecuteNonQuery("Update MRF_M set App1_By = '" & trim(request.cookies("U_ID").value) & "',App1_Date = '" & now & "',App1_Status = 'Y',App1_Rem = '" & replace(trim(txtRem.text),"'","`") & "' where MRF_NO = '" & trim(lblMRFNo.text) & "'")
                elseif rbReject.checked = true then
                    ReqCOM.ExecuteNonQuery("Update MRF_M set App1_By = '" & trim(request.cookies("U_ID").value) & "',App1_Date = '" & now & "',App1_Status = 'N',App1_Rem = '" & replace(trim(txtRem.text),"'","`") & "',MRF_Status = 'REJECTED' where MRF_NO = '" & trim(lblMRFNo.text) & "'")
                end if
    
                ShowAlert("Selected MRF has been submitted.")
                redirectPage("MRFApp1Det.aspx?ID=" & Request.params("ID"))
            End Sub
                                                    ...
                                                    <asp:DataGrid id="dtgShortage" runat="server" width="100%" Height="35px" Font-Names="Verdana" BorderColor="Black" GridLines="Vertical" cellpadding="4" Font-Name="Verdana" Font-Size="XX-Small" AutoGenerateColumns="False" OnItemDataBound="FormatRow" OnSelectedIndexChanged="GridControl1_SelectedIndexChanged">
                                                        ...
                                                        <Columns>
                                                            ...
                                                            <asp:TemplateColumn HeaderText="Qty Return">
                                                                <ItemTemplate>
                                                                    <asp:Label id="QtyReturn" runat="server" cssclass="OutputText" align="right" columns="8" maxlength="6" text='<%# DataBinder.Eval(Container.DataItem, "Qty_Return") %>' /> 
                                                                </ItemTemplate>
                                                            </asp:TemplateColumn>
