...
<script runat="server">
    ...
    Protected Sub FormatRow(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.DataGridItemEventArgs)
        ...
        If e.Item.ItemType = ListItemType.Item Or e.Item.ItemType = ListItemType.AlternatingItem Then
            ...
            Var.text = format(cdec(cdec(TotalCostN.text) - cdec(TotalCostC.text)),"##,##0.00000")
    ...
    Sub ShowDetails(s as object,e as DataGridCommandEventArgs)
        ...
        if e.commandArgument = "BOMCost" then
            if trim(FECNModelNo) = trim(ModelNo.text) then
                ...
                Script.Append("pupUp=window.open('popupReportViewer.aspx?RptName=FECNBomCostWAC&ModelNo=" & trim(ModelNo.text) & "&Revision=" & cdec(e.item.cells(5).text) & "','','toolbar=0,scrollbars=1,location=0,statusbar=0,menubar=0,resizable=1,width=700,height=250');")
    ...
    public function GetBomCost(FECNNo as string) as decimal
        ...
        ReqCom.ExecuteNonQuery("update FECN_BOM_COMPARISON set FECN_BOM_COMPARISON.std_cost_a = part_master.WAC_COST from FECN_BOM_COMPARISON,part_master where FECN_BOM_COMPARISON.part_no = part_master.part_no")

        'ReqCom.ExecuteNonQuery("update FECN_BOM_COMPARISON set FECN_BOM_COMPARISON.std_cost_a = part_master.WAC_COST from FECN_BOM_COMPARISON,part_master where FECN_BOM_COMPARISON.part_no = part_master.part_no")

        ReqCom.ExecuteNonQuery("update FECN_BOM_COMPARISON set FECN_BOM_COMPARISON.std_cost_a = part_source.UP from FECN_BOM_COMPARISON,part_source where FECN_BOM_COMPARISON.std_cost_a = 0 and FECN_BOM_COMPARISON.part_no = part_source.part_no and part_source.ven_seq = 1")
