...
<script runat="server">
    ...
    Sub Page_Load(ByVal sender As Object, ByVal e As EventArgs)
        ...
        Sub ProcLoadGridData()
            ...
            ReqCom.ExecuteNonQuery("update part_master set launch = 'Y' where std_cost_rd = 0 and part_no in (select part_no from bom_d where model_no in (Select model_no from fecn_m));")
            ...
            'if trim(cmbFECNStatus.selecteditem.value) = "ALL" then StrSql = "SELECT * FROM FECN_M where App3_Date is not null AND " & trim(cmbSearch.selecteditem.value) & " like '%" & trim(txtSearch.text) & "%'  order by seq_no desc"
            'if trim(cmbFECNStatus.selecteditem.value) <> "ALL" then StrSql = "SELECT * FROM FECN_M where App3_Date is not null AND " & trim(cmbSearch.selecteditem.value) & " like '%" & trim(txtSearch.text) & "%' AND FECN_STATUS = '" & TRIM(cmbFECNStatus.selecteditem.value) & "' order by seq_no desc"
        ...
        Protected Sub FormatRow(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.DataGridItemEventArgs)
            ...
            If e.Item.ItemType = ListItemType.Item Or e.Item.ItemType = ListItemType.AlternatingItem Then
                ...
                if ReqCOm.FuncCheckDuplicate("Select Part_No from Part_Master where std_cost_rd = 0 and part_no in (Select Part_No from BOM_D where Model_No = '" & trim(ModelNo.text) & "')","Part_No") = true then
        ...
        Sub ItemCommand(sender as Object,e as DataGridCommandEventArgs)
            ...
            if ucase(e.commandArgument) = "VIEW" then Response.redirect("FECNApp5DetView.aspx?ID=" & clng(SeqNo.text))
        end sub
                                                    ...
                                                    &nbsp;<asp:DataGrid id="GridControl1" runat="server" width="100%" OnItemDataBound="FormatRow" OnSelectedIndexChanged="GridControl1_SelectedIndexChanged" PagerStyle-HorizontalAligh="Right" PagerStyle-PrevPageText="Prev" PagerStyle-NextPageText="Next" PageSize="20" OnPageIndexChanged="OurPager" AllowPaging="True" BorderColor="Black" GridLines="None" cellpadding="4" ShowFooter="True" AutoGenerateColumns="False" OnSortCommand="SortGrid" OnItemCommand="ItemCommand">
                                                        ...
                                                        <Columns>
                                                            ...
                                                            <asp:TemplateColumn HeaderText="FECN No">
                                                                <ItemTemplate>
                                                                    <asp:Label id="FECNNo" runat="server" text='<%# DataBinder.Eval(Container.DataItem, "FECN_NO") %>' /> <asp:Label id="SeqNo" runat="server" text='<%# DataBinder.Eval(Container.DataItem, "Seq_NO") %>' visible= "false" /> 
                                                                </ItemTemplate>
                                                            </asp:TemplateColumn>
