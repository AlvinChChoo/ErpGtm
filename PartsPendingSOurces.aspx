...
<script runat="server">
    ...
    Sub cmdGo_Click(sender As Object, e As EventArgs)
        ...
        if rbByDate.checked = true then
            ...
            ReqCOM.executeNonQuery("update part_master set ind = 'Y' where create_date between '" & cdate(StartDate) & "' and '" & cdate(EndDate) & "' and part_no not in (select distinct(part_no) from part_source)")
            ReqCOM.ExecuteNonQuery("UPDATE PART_MASTER SET PENDING_STATUS = 'PENDING APPROVAL' WHERE IND = 'Y' AND PART_NO IN (SELECT DISTINCT(PART_NO) FROM UPAS_D WHERE UPAS_NO IN (select distinct(upas_no) from upas_m where upas_status = 'PENDING APPROVAL'))")
            ReqCOM.ExecuteNonQuery("update part_master set pending_status = 'PENDINS SUBMISSION' WHERE IND = 'Y' AND pending_status IS NULL")
            ShowReport("ReportViewer.aspx?RptName=PartsPendingSources&ReturnURL=Default.aspx")
            redirectPage("PartsPendingSources.aspx")
        elseif rbByPart.checked = true then
            ReqCOM.executeNonQuery("Update Part_Master set pending_status=null")
            ReqCOM.executeNonQuery("update part_master set ind = 'N'")
            ReqCOM.executeNonQuery("update part_master set ind = 'Y' where  create_date > '05/15/06' and part_no between '" & trim(txtPartFrom.text) & "' and '" & trim(txtPartTo.text) & "' and part_no not in (select distinct(part_no) from part_source)")
            ReqCOM.ExecuteNonQuery("UPDATE PART_MASTER SET PENDING_STATUS = 'PENDING APPROVAL' WHERE IND = 'Y' AND PART_NO IN (SELECT DISTINCT(PART_NO) FROM UPAS_D WHERE UPAS_NO IN (select distinct(upas_no) from upas_m where upas_status = 'PENDING APPROVAL'))")
            ReqCOM.ExecuteNonQuery("update part_master set pending_status = 'PENDINS SUBMISSION' WHERE IND = 'Y' AND pending_status IS NULL")
            ShowReport("ReportViewer.aspx?RptName=PartsPendingSources&ReturnURL=Default.aspx")
            redirectPage("PartsPendingSources.aspx")
        elseif rbByMFG.checked = true then
            ReqCOM.executeNonQuery("Update Part_Master set pending_status=null")
            ReqCOM.executeNonQuery("update part_master set ind = 'N'")
            ReqCOM.executeNonQuery("update part_master set ind = 'Y' where create_date > '05/15/06' and m_part_no + MFG like '%" & trim(txtMFG.text) & "%' and part_no not in (select distinct(part_no) from part_source)")
            ReqCOM.ExecuteNonQuery("UPDATE PART_MASTER SET PENDING_STATUS = 'PENDING APPROVAL' WHERE IND = 'Y' AND PART_NO IN (SELECT DISTINCT(PART_NO) FROM UPAS_D WHERE UPAS_NO IN (select distinct(upas_no) from upas_m where upas_status = 'PENDING APPROVAL'))")
            ReqCOM.ExecuteNonQuery("update part_master set pending_status = 'PENDINS SUBMISSION' WHERE IND = 'Y' AND pending_status IS NULL")
            ShowReport("ReportViewer.aspx?RptName=PartsPendingSources&ReturnURL=Default.aspx")
            redirectPage("PartsPendingSources.aspx")
        elseif rbByModel.checked = true then
            if ReqCOM.FuncCheckDuplicate("Select top 1 Revision from BOM_M where Model_No = '" & trim(txtModel.text) & "';","Revision") = true then
                BOMRev = ReqCOM.GetFieldVal("Select top 1 Revision from BOM_M where Model_No = '" & trim(txtModel.text) & "';","Revision")
                ReqCOM.executeNonQuery("Update Part_Master set pending_status=null")
                ReqCOM.executeNonQuery("update part_master set ind = 'N'")
                ReqCOM.executeNonQuery("update part_master set ind = 'Y' where create_date > '05/15/06' and m_part_no + MFG like '%" & trim(txtMFG.text) & "%' and part_no not in (select distinct(part_no) from part_source where part_no in (Select Part_No from BOM_D where model_no = '" & trim(txtModel.text) & "' and revision = " & cdec(BOMRev) & "))")
                ReqCOM.ExecuteNonQuery("UPDATE PART_MASTER SET PENDING_STATUS = 'PENDING APPROVAL' WHERE IND = 'Y' AND PART_NO IN (SELECT DISTINCT(PART_NO) FROM UPAS_D WHERE UPAS_NO IN (select distinct(upas_no) from upas_m where upas_status = 'PENDING APPROVAL'))")
                ReqCOM.ExecuteNonQuery("update part_master set pending_status = 'PENDINS SUBMISSION' WHERE IND = 'Y' AND pending_status IS NULL")
                ShowReport("ReportViewer.aspx?RptName=PartsPendingSources&ReturnURL=Default.aspx")
                redirectPage("PartsPendingSources.aspx")
                    ...
                    <tr>
                        <td>
                            <div align="center"><asp:Label id="Label3" runat="server" cssclass="FormDesc" width="100%">PARTS
                                PENDING UPA</asp:Label>
                            </div>
                            <div align="center">
                            </div>
                            <div align="center">
                                <asp:CustomValidator id="ValDateInput" runat="server" CssClass="ErrorText" Width="100%" ForeColor=" " Display="Dynamic" ErrorMessage="You don't seem to have supplied a valid date format." EnableClientScript="False" OnServerValidate="ValDateInput_ServerValidate"></asp:CustomValidator>
                            </div>
                            <p>
                                <table style="FONT-SIZE: xx-small; BORDER-LEFT-COLOR: black; BORDER-BOTTOM-COLOR: black; WIDTH: 60%; BORDER-TOP-COLOR: black; FONT-FAMILY: Verdana; BORDER-COLLAPSE: collapse; BORDER-RIGHT-COLOR: black" align="center" border="1">
                                    <tbody>
                                        ...
                                        <tr>
                                            <td bgcolor="silver">
                                                <asp:RadioButton id="rbByModel" runat="server" GroupName="GP1" CssClass="OutputText" Text="by Model"></asp:RadioButton>
                                            </td>
                                            <td>
