...
<script runat="server">
    ...
    Sub Page_Load(ByVal sender As Object, ByVal e As EventArgs)
        ...
        Sub ShowReport()
            ...
            Dim RptnAME as string = "IQCLedger"
            ...
            '    Case "MODEL" : CrystalReportViewer1.SelectionFormula = "{so_model_m.model_no} >= '" & trim(request.params("ModelFrom")) & "' and {so_model_m.model_no} <= '" & trim(request.params("ModelTo")) & "' and {so_model_m.so_date} >= #" & cdate(request.params("DateFrom")) & "# and {so_model_m.so_date} <= #" & cdate(request.params("DateTo")) & "#"
            '    Case "CUST" : CrystalReportViewer1.SelectionFormula = "{so_model_m.cust_code} >= '" & trim(request.params("CustFrom")) & "' and {so_model_m.cust_code} <= '" & trim(request.params("CustTo")) & "' and {so_model_m.so_date} >= #" & cdate(request.params("DateFrom")) & "# and {so_model_m.so_date} <= #" & cdate(request.params("DateTo")) & "#"
            '    Case "DATE" : CrystalReportViewer1.SelectionFormula = "{so_model_m.so_date} >= #" & cdate(request.params("DateFrom")) & "# and {so_model_m.so_date} <= #" & cdate(request.params("DateTo")) & "#"
            'End select
